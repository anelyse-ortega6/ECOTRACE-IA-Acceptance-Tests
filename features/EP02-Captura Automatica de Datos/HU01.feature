Feature: Escaneo y registro automático de facturas mediante OCR
  ...
  Scenario: Registro exitoso de una factura
    ...
    When captura una fotografía de una factura con esta información
        | Campo             | Valor          |
        | Empresa           | PetroPerú      |
        | RUC               | 20100070970    |
        | Fecha             | 15/07/2026     |
  ...
  Scenario: La factura presenta una imagen de baja calidad
    ...
    When la imagén presenta estos valores
          | Campo            | Valor      |
          | Empresa          | PetroPerú  |
          | Calidad Imagen   | Borrosa    |
  ...
  Scenario: Se intenta escanear un documento que no es una factura
    ...
    When el OCR identifica que el documento con esta informacion
      | Campo            | Valor         |
      | Tipo Documento   | Recibo Simple |
      | Estado           | No válido     |
